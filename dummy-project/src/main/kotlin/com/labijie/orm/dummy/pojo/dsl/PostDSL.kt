...
public object PostDSL {
  ...
  private fun <T> Post.getColumnValueString(column: Column<T>): String = when(column) {
    PostTable.title->this.title.orEmpty()
    PostTable.description->this.description
    PostTable.dateTime->this.dateTime.toString()

    PostTable.duration->com.labijie.infra.orm.ExposedConverter.durationToString(this.duration)

    PostTable.time->this.time.toString()

    PostTable.date->this.date.toString()

    PostTable.timestamp->com.labijie.infra.orm.ExposedConverter.instantToString(this.timestamp)

    PostTable.id->this.id.toString()

    else->throw
        IllegalArgumentException("""Can ot converter value of Post::${column.name} to string.""")
  }

  @kotlin.Suppress("UNCHECKED_CAST")
  private fun <T> parseColumnValue(valueString: String, column: Column<T>): T {
    val value = when(column) {
      PostTable.title -> valueString
      PostTable.description -> valueString
      PostTable.dateTime ->com.labijie.infra.orm.ExposedConverter.stringToLocalDateTime(valueString)
      PostTable.duration ->com.labijie.infra.orm.ExposedConverter.stringToDuration(valueString)
      PostTable.time ->com.labijie.infra.orm.ExposedConverter.stringToLocalTime(valueString)
      PostTable.date ->com.labijie.infra.orm.ExposedConverter.stringToLocalDate(valueString)
      PostTable.timestamp ->com.labijie.infra.orm.ExposedConverter.stringToInstant(valueString)
      PostTable.id ->valueString.toLong()
      else->throw
          IllegalArgumentException("""Can ot converter value of Post::${column.name} to string.""")
    }
    return value as T
  ...
  public fun PostTable.insertIgnore(raw: Post): InsertStatement<Long> = insertIgnore {
    assign(it, raw)
  }
  ...
  public fun PostTable.batchUpsert(
    list: Iterable<Post>,
    onUpdateExclude: List<Column<*>>? = null,
    onUpdate: (UpsertBuilder.(UpdateStatement) -> Unit)? = null,
    shouldReturnGeneratedValues: Boolean = false,
  ...
  public fun PostTable.selectForwardByPrimaryKey(
    ...
    val list = sorted.limit(pageSize + 1).toPostList(*selective.toTypedArray()).toMutableList()
