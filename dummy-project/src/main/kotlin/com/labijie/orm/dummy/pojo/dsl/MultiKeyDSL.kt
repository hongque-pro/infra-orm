...
public object MultiKeyDSL {
  ...
  private fun <T> MultiKey.getColumnValueString(column: Column<T>): String = when(column) {
    MultiKeyTable.key1->this.key1
    MultiKeyTable.key2->this.key2.toString()

    else->throw
        IllegalArgumentException("""Can ot converter value of MultiKey::${column.name} to string.""")
  }

  @kotlin.Suppress("UNCHECKED_CAST")
  private fun <T> parseColumnValue(valueString: String, column: Column<T>): T {
    val value = when(column) {
      MultiKeyTable.key1 -> valueString
      MultiKeyTable.key2 ->valueString.toInt()
      else->throw
          IllegalArgumentException("""Can ot converter value of MultiKey::${column.name} to string.""")
    }
    return value as T
  ...
  public fun MultiKeyTable.insertIgnore(raw: MultiKey): InsertStatement<Long> = insertIgnore {
    assign(it, raw)
  }
  ...
  public fun MultiKeyTable.batchUpsert(
    list: Iterable<MultiKey>,
    onUpdateExclude: List<Column<*>>? = null,
    onUpdate: (UpsertBuilder.(UpdateStatement) -> Unit)? = null,
    shouldReturnGeneratedValues: Boolean = false,
    ...
    val rows =  batchUpsert(data = list, keys = arrayOf(key1, key2), onUpdate = onUpdate,
      ...
      data: MultiKey-> assign(this, data)
    }
    return rows
  }
