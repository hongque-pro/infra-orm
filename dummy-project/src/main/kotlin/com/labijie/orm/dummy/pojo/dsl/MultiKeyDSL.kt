...
public object MultiKeyDSL {
  ...
  private fun <T> MultiKey.getColumnValueString(column: Column<T>): String = when(column) {
    ...
    else->throw IllegalArgumentException("""Can't converter value of MultiKey::${column.name} to string.""")
  }
  ...
  public fun Iterable<ResultRow>.toMultiKeyList(vararg selective: Column<*>): List<MultiKey> = this.map {
  ...
  public fun UpdateBuilder<*>.setValue(raw: MultiKey, vararg ignore: Column<*>): Unit = assign(this, raw, ignore = ignore)

  public fun UpdateBuilder<*>.setValueSelective(raw: MultiKey, vararg selective: Column<*>): Unit = assign(this, raw, selective = selective)
  ...
  public fun MultiKeyTable.updateByPrimaryKey(raw: MultiKey, vararg selective: Column<*>): Int = update(raw, selective = selective, ignore = arrayOf(key1, key2)) {
  ...
  public fun MultiKeyTable.selectMany(vararg selective: Column<*>, `where`: Query.() -> Query?): List<MultiKey> {
    val query = selectSlice(*selective)
    `where`.invoke(query)
    return query.toMultiKeyList(*selective)
  }

  public fun MultiKeyTable.selectOne(vararg selective: Column<*>, `where`: Query.() -> Query?): MultiKey? {
    val query = selectSlice(*selective)
    `where`.invoke(query)
    return query.firstOrNull()?.toMultiKey(*selective)
  }
