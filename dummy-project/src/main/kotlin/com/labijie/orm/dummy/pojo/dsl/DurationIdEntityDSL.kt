...
public object DurationIdEntityDSL {
  ...
  public fun <T> DurationIdEntityTable.getColumnType(column: Column<T>): KClass<*> = when(column) {
    name->String::class
    id->Duration::class
    else->throw IllegalArgumentException("""Unknown column <${column.name}> for 'DurationIdEntity'""")
  }
  ...
  private fun <T> DurationIdEntity.getColumnValueString(column: Column<T>): String = when(column) {
    ...
    else->throw IllegalArgumentException("""Can't converter value of DurationIdEntity::${column.name} to string.""")
  }
  ...
  public fun Iterable<ResultRow>.toDurationIdEntityList(vararg selective: Column<*>): List<DurationIdEntity> = this.map {
  ...
  public fun UpdateBuilder<*>.setValue(raw: DurationIdEntity, vararg ignore: Column<*>): Unit = assign(this, raw, ignore = ignore)

  public fun UpdateBuilder<*>.setValueSelective(raw: DurationIdEntity, vararg selective: Column<*>): Unit = assign(this, raw, selective = selective)

  public fun DurationIdEntityTable.insert(raw: DurationIdEntity): InsertStatement<Number> = insert {
    assign(it, raw)
  }

  public fun DurationIdEntityTable.insertIgnore(raw: DurationIdEntity): InsertStatement<Long> = insertIgnore {
    assign(it, raw)
  }
  ...
  public fun DurationIdEntityTable.updateByPrimaryKey(raw: DurationIdEntity, vararg selective: Column<*>): Int = update(raw, selective = selective, ignore = arrayOf(id)) {
    DurationIdEntityTable.id.eq(raw.id)
  }

  public fun DurationIdEntityTable.updateByPrimaryKey(id: Duration, builder: DurationIdEntityTable.(UpdateStatement) -> Unit): Int = update({ DurationIdEntityTable.id.eq(id) }, body = builder)

  public fun DurationIdEntityTable.deleteByPrimaryKey(id: Duration): Int = deleteWhere {
    DurationIdEntityTable.id.eq(id)
  }

  public fun DurationIdEntityTable.selectByPrimaryKey(id: Duration, vararg selective: Column<*>): DurationIdEntity? {
  ...
  public fun DurationIdEntityTable.selectByPrimaryKeys(ids: Iterable<Duration>, vararg selective: Column<*>): List<DurationIdEntity> {
    val query = selectSlice(*selective).andWhere {
  ...
  public fun DurationIdEntityTable.selectMany(vararg selective: Column<*>, `where`: Query.() -> Query?): List<DurationIdEntity> {
    val query = selectSlice(*selective)
    `where`.invoke(query)
    return query.toDurationIdEntityList(*selective)
  }

  public fun DurationIdEntityTable.selectOne(vararg selective: Column<*>, `where`: Query.() -> Query?): DurationIdEntity? {
    val query = selectSlice(*selective)
    `where`.invoke(query)
    return query.firstOrNull()?.toDurationIdEntity(*selective)
  }
  ...
  public fun DurationIdEntityTable.selectForwardByPrimaryKey(
    ...
    val list = sorted.limit(pageSize + 1).toDurationIdEntityList(*selective.toTypedArray()).toMutableList()
  ...
  public fun DurationIdEntityTable.replace(raw: DurationIdEntity): ReplaceStatement<Long> = replace {
    assign(it, raw)
  }
}
