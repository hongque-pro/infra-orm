...
public object DescribeEnumDSL {
  ...
  private fun <T> DescribeEnum.getColumnValueString(column: Column<T>): String = when(column) {
    DescribeEnumTable.key1->this.key1
    DescribeEnumTable.key2->this.key2.toString()

    else->throw
        IllegalArgumentException("""Can ot converter value of DescribeEnum::${column.name} to string.""")
  }

  @kotlin.Suppress("UNCHECKED_CAST")
  private fun <T> parseColumnValue(valueString: String, column: Column<T>): T {
    val value = when(column) {
      DescribeEnumTable.key1 -> valueString
      DescribeEnumTable.key2 ->valueString.toInt()
      else->throw
          IllegalArgumentException("""Can ot converter value of DescribeEnum::${column.name} to string.""")
    }
    return value as T
  ...
  public fun DescribeEnumTable.insertIgnore(raw: DescribeEnum): InsertStatement<Long> =
      insertIgnore {
    assign(it, raw)
  }
  ...
  public fun DescribeEnumTable.batchUpsert(
    list: Iterable<DescribeEnum>,
    onUpdateExclude: List<Column<*>>? = null,
    onUpdate: (UpsertBuilder.(UpdateStatement) -> Unit)? = null,
    shouldReturnGeneratedValues: Boolean = false,
    ...
    val rows =  batchUpsert(data = list, keys = arrayOf(key1, key2), onUpdate = onUpdate,
        onUpdateExclude = onUpdateExclude, where = where, shouldReturnGeneratedValues =
        shouldReturnGeneratedValues) {
      data: DescribeEnum-> assign(this, data)
    }
    return rows
  }
