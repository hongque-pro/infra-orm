...
public object DescribeEnumDSL {
  ...
  private fun <T> DescribeEnum.getColumnValueString(column: Column<T>): String = when(column) {
    ...
    else->throw IllegalArgumentException("""Can't converter value of DescribeEnum::${column.name} to string.""")
  }
  ...
  public fun Iterable<ResultRow>.toDescribeEnumList(vararg selective: Column<*>): List<DescribeEnum> = this.map {
  ...
  public fun UpdateBuilder<*>.setValue(raw: DescribeEnum, vararg ignore: Column<*>): Unit = assign(this, raw, ignore = ignore)

  public fun UpdateBuilder<*>.setValueSelective(raw: DescribeEnum, vararg selective: Column<*>): Unit = assign(this, raw, selective = selective)

  public fun DescribeEnumTable.insert(raw: DescribeEnum): InsertStatement<Number> = insert {
    assign(it, raw)
  }

  public fun DescribeEnumTable.insertIgnore(raw: DescribeEnum): InsertStatement<Long> = insertIgnore {
    assign(it, raw)
  }
  ...
  public fun DescribeEnumTable.batchUpsert(
    ...
    val rows =  batchUpsert(data = list, keys = arrayOf(key1, key2), onUpdate = onUpdate, onUpdateExclude = onUpdateExclude, where = where, shouldReturnGeneratedValues = shouldReturnGeneratedValues) {
  ...
  public fun DescribeEnumTable.updateByPrimaryKey(raw: DescribeEnum, vararg selective: Column<*>): Int = update(raw, selective = selective, ignore = arrayOf(key1, key2)) {
  ...
  ): Int = update({ DescribeEnumTable.key1.eq(key1) and DescribeEnumTable.key2.eq(key2) }, body = builder)
  ...
  public fun DescribeEnumTable.selectMany(vararg selective: Column<*>, `where`: Query.() -> Query?): List<DescribeEnum> {
    val query = selectSlice(*selective)
    `where`.invoke(query)
    return query.toDescribeEnumList(*selective)
  }

  public fun DescribeEnumTable.selectOne(vararg selective: Column<*>, `where`: Query.() -> Query?): DescribeEnum? {
    val query = selectSlice(*selective)
    `where`.invoke(query)
    return query.firstOrNull()?.toDescribeEnum(*selective)
  }
